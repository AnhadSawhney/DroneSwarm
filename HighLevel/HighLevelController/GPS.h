//Assume GPS is configured correctly
#include <UbxGpsNavPvt.h>

#define READY_TRIES 5 //how many times to ask GPS for new info
...
void GPS_Init() { 
  ...
  for(int i = 0, !gps.ready(), i++) {
    if(i >= READY_TRIES) {
      baselineAltitude = 0; //failed to get altitude, assume sea level
      return;
    }
  }
  baselineAltitude = gps.height / 1000.0; //meters
}
...
void GPS_Everyloop() {
  ...
  //GPS is ready, get new data
  
}

void GPS_noinfo() { //internal function to give values when GPS is lost
  l_lat_gps = 0;
  l_lon_gps = 0;
  lat_gps_previous = 0;
  lon_gps_previous = 0;
...
void GPS_siminfo() { //simulate GPS reading from previous values in between GPS refreshes
